...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if(!tail_){
    ...
    // There are at least two items in the list. We'll compare all adjacent pairs.
    Node* prev = nullptr;
    Node* cur = head_;
    while (cur->next&&cur->data<newData) {
      // Step forward
      prev = cur;
      cur = cur->next;
    }
    std::cout << "This line is reached." << std::endl;
      //If the last item is tail, add as last.
    if(!cur->next){
      Node* oldTail = tail_;
      oldTail->next = newNode;
      newNode->prev = oldTail;
      tail_ = newNode;
      //If the item is smaller then the first item, add as first
      ...
      //Inserting between prev and cur.
      newNode->next = cur;
      newNode->prev = prev;
