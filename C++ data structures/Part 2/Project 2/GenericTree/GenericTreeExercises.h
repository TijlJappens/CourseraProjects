...
static void treeFactory(GenericTree<int>& tree) {
  ...
  root->data=4;
  GenericTree<int>::TreeNode* nextNode = root->addChild(8);
  root->addChild(15);
  GenericTree<int>::TreeNode* finalDummyNode = nextNode->addChild(16);
  nextNode->addChild(23);
  finalDummyNode->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  v.push_back({rootNodePtr});
  while(true){
    bool noChildren=false;
    for(auto n : v[v.size()-1]){
      if(n->childrenPtrs.size()!=0){
      noChildren = true;
      }
    }
    if(!noChildren){break;}
    ...
    v.push_back({});
    for(TreeNode* n : v[v.size()-2]){
      for(TreeNode* m : n->childrenPtrs){
        v[v.size()-1].push_back(m);
      }
    }
  }


  for(std::vector<TreeNode*> x : v){
    for(TreeNode* y : x){
      results.push_back(y->data);
    }
  }
  //      *****************************************************
  //                           EXERCISE 2
